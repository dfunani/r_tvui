...
pub fn resolve_start_path(
    ...
    let Some(raw) = start_path else {
        return (filesystem::absolute(Path::new(".")), None, None);
    };
    ...
    if path.is_file() {
        ...
        return (filesystem::absolute(parent), Some(name), None);
    }
...
mod tests {
    ...
    fn invalid_path_falls_back_with_warning() {
        let (cwd, select, warn) =
            resolve_start_path(Some(PathBuf::from("/nonexistent-rtvui-path-xyz-99999")));
        assert!(select.is_none());
        assert!(warn.is_some());
        assert!(cwd.0.is_dir());
    }
